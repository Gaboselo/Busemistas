...
class _ConductorHomeVistaState extends State<ConductorHomeVista> {
  ...
  // ---------------------------------------------------------------------------
  // SOS listener: detecta SOS de pasajero. Anti-spam via ts_emergencia.
  // Solo muestra 1 dialogo por cada SOS nuevo (identificado por timestamp).
  // ---------------------------------------------------------------------------
  StreamSubscription<DocumentSnapshot>? _sosListener;
  int? _tsUltimoSosMostrado; // microseconds del ultimo ts_emergencia mostrado
  bool _dialogoSosVisible = false; // true mientras el dialogo esta en pantalla
  ...
  // ID de la camioneta cacheado en initState para no llamar context.read
  // dentro del listener (que se dispara en background).
  late String _cachedCamionetaId;

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      // Cachear el ID antes de lanzar el listener para evitar
      // llamar context.read() desde dentro del StreamSubscription.
      _cachedCamionetaId =
          context.read<AuthProvider>().camionetaAsignada ?? 'camioneta_01';
      await _cargarUnidad();
      _iniciarSosListener();
  ...
  // ---------------------------------------------------------------------------
  // Escucha el documento de la camioneta en tiempo real.
  // Si el campo 'tipo_emergencia' contiene "SOS Pasajero" y es un valor NUEVO,
  // muestra un diálogo de alerta al conductor.
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  // _iniciarSosListener
  //
  // Escucha cambios en el doc de la camioneta. Solo abre el dialogo SOS cuando:
  //   1. tipo_emergencia contiene "SOS Pasajero"
  //   2. ts_emergencia es un Timestamp valido
  //   3. El microsecond de ese Timestamp es DISTINTO al ultimo ya mostrado
  //   4. No hay un dialogo SOS visible ya en pantalla
  //
  // De esta forma, aunque el GPS actualice lat/lng cada 3s y el stream emita
  // decenas de veces, el dialogo solo aparece UNA vez por cada SOS nuevo.
  // ---------------------------------------------------------------------------
  void _iniciarSosListener() {
    _sosListener?.cancel();
    _sosListener = _db
        .collection("camionetas")
        .doc(_cachedCamionetaId)
        .snapshots()
        .listen((snap) {
      ...
      // Sincronizar _emergenciaActiva SIN llamar setState si no cambio
      final esEmergencia =
          estado == "emergencia" || estado == "fuera_de_servicio";
      if (esEmergencia != _emergenciaActiva && mounted) {
        setState(() => _emergenciaActiva = esEmergencia);
      }

      // Leer tipo y timestamp del SOS
      final tipoEmergencia = data["tipo_emergencia"] as String? ?? "";
      final tsEmergencia = data["ts_emergencia"];

      final esSosPasajero = tipoEmergencia.contains("SOS Pasajero");

      if (!esSosPasajero || _dialogoSosVisible) return;

      // Extraer microseconds del Timestamp para identificar unicidad
      int? tsMicros;
      if (tsEmergencia is Timestamp) {
        tsMicros = tsEmergencia.microsecondsSinceEpoch;
      }

      // Si no hay timestamp valido, usar hash del texto como fallback
      final idSos = tsMicros ?? tipoEmergencia.hashCode;

      if (idSos == _tsUltimoSosMostrado) return; // mismo SOS, ya mostrado

      _tsUltimoSosMostrado = idSos;
      _dialogoSosVisible = true;
      _mostrarAlertaSosPasajero(tipoEmergencia, data);
    });
  }

  // ---------------------------------------------------------------------------
  // Diálogo de alerta que aparece en el panel del conductor cuando un
  // pasajero activa el botón SOS desde la pantalla de viaje activo.
  // ---------------------------------------------------------------------------
  void _mostrarAlertaSosPasajero(
      String tipoEmergencia, Map<String, dynamic> data) {
    if (!mounted) return;
    // Extraer ubicacion actual de la unidad
    final lat = (data["latitud"] as num?)?.toDouble();
    final lng = (data["longitud"] as num?)?.toDouble();
    final ubicacionTexto = (lat != null && lng != null)
    ...
    showDialog(
      ...
      barrierColor: Colors.red.withValues(alpha: 0.4),
      builder: (_) => AlertDialog(
        backgroundColor: Colors.red.shade50,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        icon: const Icon(Icons.warning_rounded, color: Colors.red, size: 52),
        title: const Text(
          "🚨 SOS ACTIVADO POR PASAJERO",
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Text(
            "Un pasajero de tu unidad solicitó ayuda de emergencia. "
            "Detente de forma segura y contacta a control central.",
            textAlign: TextAlign.center,
        ...
        actions: [
          FilledButton.icon(
            ...
            label: const Text("Entendido — Atender emergencia"),
            onPressed: () {
              // Marcar que el dialogo ya no esta visible para permitir
              // un nuevo SOS en el futuro (si hay otro pasajero diferente).
              if (mounted) setState(() => _dialogoSosVisible = false);
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snap) {
        ...
        // Preferir el GeoPoint "ubicacion"; si no existe usar lat/lng separados
        // (que escribe _escribirUbicacion en cada paso de la simulacion).
        double? busLat;
        double? busLng;
        final ubicacion = data["ubicacion"];
        if (ubicacion is GeoPoint) {
          busLat = ubicacion.latitude;
          busLng = ubicacion.longitude;
        } else {
          busLat = (data["latitud"] as num?)?.toDouble();
          busLng = (data["longitud"] as num?)?.toDouble();
        }
        if (busLat == null || busLng == null) return const SizedBox();
        ...
        // calcularETA devuelve los valores como String ("eta_minutos", "distancia_km")
        final minutos =
            double.tryParse(destinoData["eta_minutos"]?.toString() ?? "0") ??
                0.0;
        final km =
            double.tryParse(destinoData["distancia_km"]?.toString() ?? "0") ??
                0.0;
