  ...
  Widget build(BuildContext context) {
    ...
    for (final c in camProv.camionetas) {
      if (c.ubicacion != null) {
        ...
        // Aplicar factor de tráfico si el modelo lo reporta
        // CamionetaModelo expone trafico_denso; el nivel_trafico se lee del
        // StreamBuilder de Firestore. Usamos trafico_denso como señal rápida.
        final factorTrafico = c.trafico_denso ? 1.5 : 1.0;
        etaMap[c.id] = _calcularETAMin(dist) * factorTrafico;
      } else {
        etaMap[c.id] = null;
      }
    }
              ...
              builder: (context, snap) {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    // ── ETA calculado en tiempo real desde lat/lng fresca ──
                    // Se usa la lat/lng del documento fresco (del StreamBuilder),
                    // NO del etaMap congelado que se pasó al abrir el modal.
                    final lat = (data['latitud'] as num?)?.toDouble();
                    final lng = (data['longitud'] as num?)?.toDouble();
                    double? eta;
                    bool traficoActivo = false;
                    if (lat != null && lng != null) {
                      final esHaciaUSM =
                          destino.toLowerCase().contains('usm') ||
                              destino.toLowerCase().contains('florencia');
                      final destCoord = esHaciaUSM ? _coordUSM : _coordLaCalif;
                      final distM = Geolocator.distanceBetween(
                          lat, lng, destCoord.latitude, destCoord.longitude);
                      // Aplicar factor de tráfico si existe
                      final nivelTrafico =
                          data['nivel_trafico'] as String? ?? 'bajo';
                      final factor = nivelTrafico == 'alto'
                          ? 1.7
                          : nivelTrafico == 'medio'
                              ? 1.3
                              : 1.0;
                      traficoActivo = nivelTrafico != 'bajo' ||
                          (data['trafico_denso'] as bool? ?? false);
                      eta = (distM / 1000.0 / 40.0) * 60.0 * factor;
                    }
